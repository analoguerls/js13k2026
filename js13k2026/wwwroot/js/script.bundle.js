(function () {
  ...
  function tryAction () {
      ...
      if (unicorn && game.carrying.length < 3) {
          ...
          game.carrying.push(isBadScoop ? RAINBOW[7] : unicorn.color);
  ...
  game.loop = GameLoop({
      ...
      update (dt) {
          ...
          if (game.flies) {
              ...
              if (game.carrying.length && game.carrying[game.carrying.length - 1] !== RAINBOW[7] && dist(game.player, swarm) < FLY_CATCH_R + swarm.radius + game.player.width / 2) {
                  game.carrying[game.carrying.length - 1] = RAINBOW[7];
                  soundFx('flies');
                  showStatus(CODE_BROWN);
              }
